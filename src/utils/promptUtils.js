...
export const loadAllPrompts = async () => {
  const required = [
    "level_detector_prompt",
    "structure_validator_prompt",
    "template_validator_prompt",
...
export const buildErrorResponse = (promptName, errorMessage) => {
  switch (promptName) {
    case "level_detector_prompt":
      return JSON.stringify({ level: "unknown", type: "unknown" });
    case "structure_validator_prompt":
    case "template_validator_prompt":
      return JSON.stringify({
        issues: [`Error processing response: ${errorMessage}`],
        recommendations: ["Try again or check the file manually"],
        valid: false,
      });
    case "security_scanner_prompt":
      return JSON.stringify({
        criticalIssues: [],
        highIssues: [`Error processing response: ${errorMessage}`],
        mediumIssues: [],
        lowIssues: [],
        hasSecurity: true,
      });
